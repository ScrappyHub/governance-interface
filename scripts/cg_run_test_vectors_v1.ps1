...
foreach($cf in $caseFiles){
  ...
  if(Test-Path -LiteralPath $expPath -PathType Leaf){ $expected=(CG-ReadUtf8NoBom $expPath | ConvertFrom-Json -ErrorAction Stop); $hasPinned=$true }
  if(-not $hasPinned){
    if(-not $GenerateExpected){ Die ("MISSING_EXPECTED: " + $expPath + " (run with -GenerateExpected once, then pin expected_generated -> expected and commit)") }
    $u=[pscustomobject]@{ schema="covenantgate.eval_expected.v1"; case_id=$id; output=$out }
    $txt=CanonOut $u; $u8=New-Object System.Text.UTF8Encoding($false); [System.IO.File]::WriteAllBytes($genPath, $u8.GetBytes($txt))
